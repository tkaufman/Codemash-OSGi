...
public class EntryDAOImplTest extends
	...
	public void create_entry() {
		...
		entryFixture = RaffleIntegrationTestHelper.constructEntry("Todd", "toddkaufman@gmail.com", raffleFixture);
	}
	...
	public void test_update_leaves_the_same_number() {
		...
		RaffleIntegrationTestHelper.flushHibernateSession(factory);
		int afterCount = super.countRowsInTable("ENTRIES");
	...
	public void test_update_changes_the_row() {
		...
		target.update(toUpdate);
		RaffleIntegrationTestHelper.flushHibernateSession(factory);
		RaffleIntegrationTestHelper.assertDbFieldsMatch(simpleJdbcTemplate, toUpdate);
	}
	...
	@Test
	public void test_find_all_returns_a_the_correct_row() {
		Entry source = target.create(entryFixture);
		List<Entry> results = target.findAll();
		assertEquals(1, results.size());
		RaffleIntegrationTestHelper.assertTwoEntriesMatch(source, results.get(0));
	}
	...
	public void test_find_all_returns_multiples() {
		for (int i=0; i<3; i++) {
			Entry e = RaffleIntegrationTestHelper.constructEntry("Name" + i, "toddkaufman@gmail.com", raffleFixture);
			target.create(e);
		}
		List<Entry> results = target.findAll();
